...
export class ChatService {
  ...
  getAuthorisation(auth: any): boolean {
    if (auth.token === undefined) return false;
  ...
  async getAllMessagesDm(all: any) {
    ...
    else name_channel = `dm${all.myUserId}`;
  ...
  async PostCreateDmMessage(message: any) {
    ...
    else name_channel = `dm${message.myUserId}`;
  ...
  async PostCreateMessageChannel(
  ...
  getMyUserId(socketId: string): number {
    ...
    this.onlinePlayers.forEach((value, key) => {
      value.socketId?.map((elem) => {
        if (socketId === elem) num = key.valueOf();
        return elem;
      });
    });
    return num;
  ...
  getMyName(socketId: string): string {
    ...
    this.onlinePlayers.forEach((value) => {
      value.socketId?.map((elem) => {
        if (socketId === elem) name = value.name.toString();
      });
    });
    return name;
  }
  ...
  getSocketIdWithName(name: string): string[] {
    let socketId: string[] = [];
    this.onlinePlayers.forEach((value, key) => {
      if (value.name === name) socketId = [...value.socketId];
    });
    return socketId;
  }
  ...
  async updateChannelPass(
    ...
    if (checkIfOwnerOfChannel.owner !== userId) return;
  ...
  async muteToChannel(
    ...
    try {
      ...
      if (chan.inChannel?.some((elem) => elem.id === user.id) === false)
        return undefined;
      if (user.id === chan.owner) return undefined;
      if (
        chan.administrators?.some((elem) => elem.id === user.id) === true &&
        myUserId !== chan.owner
      )
        return undefined;
